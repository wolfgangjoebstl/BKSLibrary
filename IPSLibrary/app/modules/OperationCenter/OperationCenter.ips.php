...
if (($_IPS['SENDER']=="Execute") && $ExecuteExecute)
	...
	*************************************************************************************/
    ...
    if (false)
        ...
        if ( $eventListforDeletion && (count($eventListforDeletion)>0) )
            {
    ...
    $autosteuerung_config=Autosteuerung_GetEventConfiguration();            // events die eine Action auslösen

    $filter="IPSMessageHandler_Event";
    if (false)
        {
        $eventListfromIPS = $testMovement->getEventListfromIPS($filter,true);
        $html=$testMovement->getComponentEventListTable($eventListfromIPS,$filter,true,true);
        echo $html;
        echo "\n";
        $debug=true;
        }
    ...
    if (false)
        {
        echo "TestMovement->getAutoEventListTable mit debug abgeschlossen =====================================================================\n";
	...
	switch ($_IPS['EVENT'])
		...
		case $tim7ID:       // FileStatusTimer
			 ...
			 * StatusInformation von sendstatus auf ein Dropboxverzeichnis kopieren
	   		 ...
	   		 *
			 *************************************************************************************/
            IPSUtils_Include ("SendStatus_Library.class.php","IPSLibrary::app::modules::OperationCenter");             // nur laden wenn benötigt
