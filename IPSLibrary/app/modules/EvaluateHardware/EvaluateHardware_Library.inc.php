 ...
 *  ShowEvaluateHardware                Webfront Darstellungs Unterstützung
 *  ImproveDeviceDetection
 *  EvaluateHardware                    getdeviceHomematic/Shelly writes EvaluateHardware_include
 *  ExcelHandler
 *
 *
 */
...
class EvaluateHardware
    ...
    *  Shelly Geräte
    *
    *   getShellyInstances              grosses array für Instanzen : ShellyDevices und ShellyComponents
    *   getShellyDevices
    *
    ******************************************/
  

    function getShellyInstances(&$includeshelly)
        {
        $ShellyList=array();
        $modulhandling = new ModuleHandling();
        $devices=$modulhandling->getInstances("ShellyDevice");
        $includeshelly .= '/* ShellyList function automatically generated by EvaluateHardware::getShellyInstances on '.date("d.m.Y H:i:s").'  */'."\n\n";
        $includeshelly .=	'function ShellyList() {'."\n".'            return array('." \n";
        if (isset($this->installedModules["DetectMovement"]))     $DetectDeviceHandler = new DetectDeviceHandler(); 
        if (isset($this->installedModules["OperationCenter"]))    
            {
            IPSUtils_Include ('DeviceManagement_Library.class.php', 'IPSLibrary::app::modules::OperationCenter');                
            $DeviceManager = new DeviceManagement_Shelly(false);            // false,true,2 Debug Level
            }
        echo "\nShelly Instanzen von Geräten (ShellyDevice): ".sizeof($devices)."\n\n";
        //echo "getShellyInstances (ShellyDevice):\n";
        foreach ($devices as $instanz)
            {
            $type="";
            ...
            $connectionId = $instance["ConnectionID"];            // MQTT Server Splitter

            $instanzName=IPS_GetName($instanz);
            $ShellyList[$instanzName]["OID"]=$instanz;
            $ShellyList[$instanzName]["MQTTTopic"]=$mqttTopic;
            $ShellyList[$instanzName]["ModelID"]=$modelId;
            $ShellyList[$instanzName]["Name"]=IPS_GetName($instanz);
            $ShellyList[$instanzName]["Server"]=IPS_GetName($connectionId);
            $ShellyList[$instanzName]["ServerId"]=$connectionId;
            
            if ($DeviceManager)
                {
                // adds Shelly Devices to itemlist that are not part of ConfigurationManager
                $entry=array();
                $entry["OID"]=$instanz;
                $type=$DeviceManager->getDeviceType($instanz,false,$entry,false);       //false,true,2 max Debug
                }

            echo "   ".str_pad($instanz,8).str_pad(IPS_GetName($instanz),30);
            echo str_pad($modelId,20);
            echo str_pad($mqttTopic,40);
            ...
            $includeshelly.="\n         ".'"Name" => "'.IPS_GetName($instanz).'", ';
            $includeshelly.="\n         ".'"ModelID" => "'.$modelId.'", ';
            $includeshelly.="\n         ".'"Server" => "'.IPS_GetName($connectionId).'", ';
            ...
            foreach($cids as $cid)
                ...
                //echo "\nCID :".$cid;
                //print_r($o);
                if($o['ObjectIdent'] != "")                 // Auswertung auf 
                    {
                    $includeshelly.="\n                ".'"'.$o['ObjectIdent'].'" => array(';
                    $includeshelly.="\n                              ".'"OID" => "'.$o['ObjectID'].'", ';
                    $includeshelly.="\n                              ".'"Name" => "'.$o['ObjectName'].'", ';
        ...
        $components=$modulhandling->getInstances("ShellyComponent");
        //print_r($components);
        
        echo "\nShelly Instanzen von Komponenten (ShellyComponent): ".sizeof($components)."\n\n";
        //echo "ShellyComponent (Component Scopy of each Device)\n";           // from Configurator
        foreach ($components as $instanz)
            {
            $type="";
            $config=json_decode(IPS_GetConfiguration($instanz),true);
            $mqttTopic = $config["MQTTTopic"];
            ...
            $ShellyList[$instanzName]["OID"]=$instanz;
            $ShellyList[$instanzName]["MQTTTopic"]=$mqttTopic;
            $ShellyList[$instanzName]["Component"]=$component;
            $ShellyList[$instanzName]["Channel"]=$channel;
            $ShellyList[$instanzName]["Name"]=IPS_GetName($instanz);
            $ShellyList[$instanzName]["Server"]=IPS_GetName($connectionId);
            $ShellyList[$instanzName]["ServerId"]=$connectionId;
            
            echo "   ".str_pad($instanz,8).str_pad(IPS_GetName($instanz),30);
            echo str_pad($mqttTopic,30);
            ...
            if ($DeviceManager)
                ...
                $entry=array();
                $entry["OID"]=$instanz;
                $fulltype=$DeviceManager->getDeviceType($instanz,4,$entry,false);       //2 max Debug
                //print_r($fulltype);
    ...
    /* EvaluateHardware::getShellyDevices
     * get the Devices only, can be Components or Devices, but only one per SerialID/MQTT Topic
     *
     */
    function getShellyDevices(&$includeshelly)
        {
        $debug=false;
        $shellyDevices=array();
        $modulhandling = new ModuleHandling();
        $devices=$modulhandling->getInstances("ShellyDevice");
        $includeshelly .= 'function getShellyConfiguration() {'."\n".'            return array('." \n";

        if (isset($this->installedModules["OperationCenter"]))    
            {
            IPSUtils_Include ('DeviceManagement_Library.class.php', 'IPSLibrary::app::modules::OperationCenter');                
            $DeviceManager = new DeviceManagement_Shelly();
            }
        if ($debug)
            {
            echo "getShellyDevices, Shelly showItemlist from DeviceManagement_Shelly:\n";
            $DeviceManager->showItemlist();
            echo "ShellyDevice (Full Scope)\n";              // MQTT
            }
        //print_r($devices);
        foreach ($devices as $oid)
            {
            $type="";
            ...
            if ($debug)
                {
                echo "   ".str_pad($oid,8).str_pad($name,30);
                echo str_pad($modelId,20);
                echo str_pad($mqttTopic,30);
                //echo json_encode($config);
                echo "\n";
                }
            //$modulName    = $instance["ModuleName"];            // Device or Component
            //print_R($instance);
            $entry=array();
            $entry["OID"]=$oid;
            $type=$DeviceManager->getDeviceType($oid,false,$entry,false);       //2 max Debug
            $includeshelly.='             '.str_pad(('"'.IPS_GetName($oid).'"'),40).' => array("'.$mqttTopic.'","'.$modelId.'","MQTT","'.$type.'"),'."\n";
            $shellyDevices[$mqttTopic][]=$name;
            }
        $components=$modulhandling->getInstances("ShellyComponent");
        //print_r($components);
        if ($debug) echo "ShellyComponent (Component Scopy of each Device)\n";           // from Configurator
        foreach ($components as $oid)
            {
            $type="";
            $config=json_decode(IPS_GetConfiguration($oid),true);
            $mqttTopic = $config["MQTTTopic"];
            ...
            $entry=array();
            $entry["OID"]=$oid;
            $fulltype=$DeviceManager->getDeviceType($oid,4,$entry,$debug);       //2 max Debug
            //print_r($fulltype);
            $type=$fulltype["TYPECHAN"];

            $includeshelly.='             '.str_pad(('"'.IPS_GetName($oid).'"'),40).' => array("'.$mqttTopic.'","'.$component.":".$channel.'","MQTT","'.$type.'"),'."\n";

            $shellyDevices[$mqttTopic][]=$name;
            }
        $includeshelly.=');}'."\n"; 
        print_R($shellyDevices);
        return(true);
        }    
