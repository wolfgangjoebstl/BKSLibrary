 ...
 * Die Liste jann entweder die HarwdareListe oder die DeviceListe aus EvaluateHardware sein, wird automatisch erkannt.
 *
 *
	...
	function getComponent($Elements,$keywords, $write="Array", $debug=false)
		...
		foreach ($Elements as $Key)
             ...
             */
                ...
                if ( is_array($keywords) == true )      // Übergabe Array
                    {
                ...
                if ($found)
                    {
                    $keyName=$Key["Name"];
                    $coid=(integer)$Key["COID"][$keyword]["OID"];
                    $oid=(integer)$Key["OID"];                                                     
                    }
            ...
            if ($found)
                ...
                */
                ...
                $component[]=(integer)$coid;
                $install[$keyName]["COID"]=(integer)$coid;
                $install[$keyName]["OID"]=$oid;
                $install[$keyName]["KEY"]=$keyword;
                ...
                $vartyp=IPS_GetVariable($coid);
                if ($this->debug)
                    {
                    if ($vartyp["VariableProfile"]!="")
                        {
                        $result .= "  ".str_pad($keyName."/".$keyword,50)." = ".GetValueFormatted($coid)."   (".date("d.m H:i",IPS_GetVariable($coid)["VariableChanged"]).")       \n";
                        }
                    else
                        {
...
class sqlHandle
    ...
    public function __construct($oid=false)
        ...
        if ($oid === false) 
            ...
            if (sizeof($oidResult)>0) $oid=$oidResult[0];           // ersten treffer newt_checkbox_tree_get_multi_selection
            else 
                {
        ...
        echo "sqlHandle, new $oid (".IPS_GetName($oid).")\n";
    ...
    public function getDatabaseConfig()
        {
        echo "getDatabaseConfig: return Config for Database ".$this->dataBase." with MariaDB:\n";
        switch ($this->dataBase)
            {
            case "ipsymcon":
                $config=array(
                    "deviceList"    =>  ["deviceID"     => ["Field" => "deviceID", "Type" => "int", "Null" => "NO", "Key" => "PRI", "Default" => "", "Extra" => "auto_increment"],
                                        "Name"         => ["Field" => "Name", "Type" => "varchar(255)", "Null" => "NO", "Key" => "UNI", "Default" => "", "Extra" => ""],
                                        "Type"         => ["Field" => "Type", "Type" => "varchar(255)", "Null" => "NO", "Key" => "", "Default" => "", "Extra" => ""],
                                        "SubType"      => ["Field" => "SubType", "Type" => "varchar(255)", "Null" => "", "Key" => "", "Default" => "", "Extra" => ""],
                                        "Serialnummer" => ["Field" => "Serialnummer", "Type" => "varchar(255)", "Null" => "", "Key" => "", "Default" => "", "Extra" => ""],
                                        "Information"  => ["Field" => "Information", "Type" => "varchar(255)", "Null" => "", "Key" => "", "Default" => "", "Extra" => ""],
                                        ],
                    "instances"      => ["instanceID"   => ["Field" => "instanceID", "Type" => "int", "Null" => "NO", "Key" => "PRI", "Default" => "", "Extra" => "auto_increment"],
                                        ...
                                        "CONFIG"       => ["Field" => "CONFIG", "Type" => "varchar(1023)", "Null" => "", "Key" => "", "Default" => "", "Extra" => ""],
                                        ],
                    "channels"       => ["channelID"     => ["Field" => "channelID", "Type" => "int", "Null" => "NO", "Key" => "PRI", "Default" => "", "Extra" => "auto_increment"],
                                        ...
                                        "RegisterAll"  => ["Field" => "RegisterAll", "Type" => "varchar(1024)", "Null" => "NO", "Key" => "UNI", "Default" => "", "Extra" => ""],            //jsonencode
                                        ],
                    "actuators"      => ["actuatorID"     => ["Field" => "actuatorID", "Type" => "int", "Null" => "NO", "Key" => "PRI", "Default" => "", "Extra" => "auto_increment"],
                                        "deviceID"     => ["Field" => "deviceID", "Type" => "int", "Null" => "NO", "Key" => "", "Default" => "", "Extra" => ""],
                                        "portID"       => ["Field" => "portID", "Type" => "int", "Null" => "NO", "Key" => "", "Default" => "", "Extra" => ""],
                                        ],
                    "registers"      => ["registerID"     => ["Field" => "registerID", "Type" => "int", "Null" => "NO", "Key" => "PRI", "Default" => "", "Extra" => "auto_increment"],
                                        "deviceID"       => ["Field" => "deviceID", "Type" => "int", "Null" => "NO", "Key" => "", "Default" => "", "Extra" => ""],
                                        "portID"         => ["Field" => "portID", "Type" => "int", "Null" => "NO", "Key" => "", "Default" => "", "Extra" => ""],
                                        "Name"           => ["Field" => "Name", "Type" => "varchar(255)", "Null" => "NO", "Key" => "UNI", "Default" => "", "Extra" => ""],
                                        "TYPEREG"        => ["Field" => "TYPEREG", "Type" => "varchar(255)", "Null" => "", "Key" => "", "Default" => "", "Extra" => ""],
                                        "Configuration"  => ["Field" => "Configuration", "Type" => "varchar(1023)", "Null" => "NO", "Key" => "UNI", "Default" => "", "Extra" => ""],          //jsonencode
                                        ],
    ...
    /* describe Tables, altuelle Config ausgeben */
    ...
    public function countTable($table,$column,$needle="*")
        {
        if ($needle=="*")
            {
    ...
    public function makeTableConfig($entry, $debug=false)
        {
    ...
    public function compareTableConfig($columnSoll,$columnIst,$debug=false)
        ...
        if ($columnSoll["Null"] != $columnIst["Null"]) 
            ...
            if (!( ($columnSoll["Null"] == "") && (strtoupper($columnIst["Null"]) == "YES") ))
                {
                $same=false;
                echo "Null not the same: ".$columnSoll["Null"]." != ".$columnIst["Null"]."\n";
                }
            }
        if ($columnSoll["Key"] != $columnIst["Key"]) 
            {
            $same=false;
            echo "Key not the same.\n";
            }
        if ($columnSoll["Default"] != $columnIst["Default"]) 
            {
            $same=false;
            echo "Default not the same.\n";
            }
        if ($columnSoll["Extra"] != $columnIst["Extra"]) 
            {
            $same=false;
            echo "Extra not the same.\n";
            }
        if ($columnSoll["Type"] != $columnIst["Type"]) 
            ...
            if ($typeSoll[0] != $typeIst[0]) 
                {
                echo "Type not the same: ".$typeSoll[0]." mit ".$typeIst[0]." not equal.\n";
                $same=false;
                }
            elseif (isset($typeSoll[1]))
                ...
                if ( (strtoupper($typeSoll[0])) != "INT")           // INT nicht überprüfen
                    ...
                    if ($typeSoll[1] != $typeIst[1]) 
                        {
                        echo "Type not the same: ".$typeSoll[0]."(".$typeSoll[1]." with ".$typeIst[0]."(".$typeIst[1]." is not the same.\n";
                        $same=false;    
                        }
                    }
     ...
     *    $name = Key/Index             Wert für den Key 
     *    deviceList                    Werte im Array die upgedatet werden sollen
     *
     *    $config                   Konfiguration dieser Tabelle, ist auch in der Class gespeichert
     *
     */

    public function updateTableValue($table,$columnValue,$name,$deviceList,$config=NULL,$debug=false)
        {
        $update=false;
        switch ($table)
            {
            case "deviceList":                  // Index deviceID, Key name
                $index = "deviceID";
                $key="Name";
                break;
            case "instances":                  // Index instanceID, Key deviceID
                $index = "instanceID";
                $key="deviceID";
                break;
            default:
        ...
        $count = $this->countTable($table,$columnValue,$name);           // SELECT COUNT(columnValue) FROM deviceList WHERE columnValue='$name';
        if ($count == 0)            // neuer Eintrag
            {
            if ($debug) echo "updateTableValue: Eintrag in Tabelle $table für $columnValue mit $name noch nicht gefunden. Neuer Eintrag:\n";
            ...
            switch ($table)
                ...
                case "deviceList":                  // Index deviceID, Key name
                    ...
                    foreach ($config as $column => $entry)
                        ...
                        if ($column==$index) echo "Index";              // Index
                        elseif ($column == $key)            // Key
                            {
                            echo "Key ($key) : $name";
                            if ($next) {$columns.=","; $values.=",";} else  $next=true;
                            $columns.= $key;            // auch == columnValue, muss aber nicht
                            $values.="'$name'";
                            }
                        else
                            {
        ...
        elseif ($count > 1)
            {
            echo "     Eintrag $name mit $count Duplicates. Delete younger ones !\n";
            ...
            {
            $sql = "SELECT * FROM $table WHERE $columnValue='$name';";
            $result1=$this->query($sql);
            $row = $result1->fetch()[0];
            $result1->result->close();                      // erst am Ende, sonst ist mysqli_result bereits weg !
            //print_r($row);
            ...
            foreach ($config as $column => $entry)
                {
                $text .= "   Look for ".str_pad($column,32);
                if (isset($deviceList[$column])) 
                    {
                    if ($row[$column] == $deviceList[$column]) $text .= "    available";
                    else
                        {
                        $update=true;
                        $text .= "    available, update needed.";
                        if ($next) $sqlCommand .= ",";
                        else $next=true; 
                        $sqlCommand .= "$column = '".$deviceList[$column]."' ";
                        }
                    }
                $text .= "\n";
                }
            if ($update)
                {
                echo "     Eintrag $name gefunden. Update notwendig:\n";
                //echo $text; 
                $sqlCommand .= "WHERE $columnValue='$name';";
                echo " >SQL Command : $sqlCommand\n";    
                $result=$this->command($sqlCommand);
                }
            }
        ...
        $result=array();
        $result["Status"]=true;
        $sqlCommand = "SELECT $index FROM $table WHERE $columnValue='$name';";
        $result1=$this->query($sqlCommand);
        $columns = $result1->fetch();
        $result1->result->close();                      // erst am Ende, sonst ist mysqli_result bereits weg !
        if (isset($columns[0][$index])) $result["Index"]=$columns[0][$index];
     ...
     *      columns         Spalten aus DESCRIBE tableName
     *      configSet       gewünschte Konfiguration, ähnlich der die ausgegeben wird
     *
     *  ALTER TABLE $tableName
     *
     */

    public function syncTableConfig($tableName, $columns, $configSet, $debug=false)
        {
        if ($debug) echo "Configurationen der Tabellen $tableName vergleichen:\n";
        if ($debug) echo "   Check Anzahl der Spalten : Ist : ".sizeof($columns)." Soll : ".sizeof($configSet)."\n";
        ...
        if ($debug) echo "      ".str_pad("Spalte",30).str_pad("Status Soll Config",20)."\n";
        foreach ($columns as $column)
            {
        ...
        foreach ($configSet as $name => $column)
            ...
            $same = $this->compareTableConfig($column,$columns[$name]);
            if ($same==false)         // Primary Key wird nicht automatisch geändert aber bei neuen Tabellen richtig angelegt 
                {
                $sqlCommand="";
    ...
    public function createTableConfig($tableName, $entries, $debug=false)
        ...
        foreach ($entries as $column => $entry)
            ...
            $result = $this->makeTableConfig($entry);
            if (isset($result[$column])) $sqlCommand.= $result[$column];
            else echo "Fehler, $column not Field Name.\n";
            /*
            $type = "varchar(255);"; $noNull = ""; $unique=""; $extra="";
            if (isset($entry["Field"]) )
        ...
        $sqlCommand.=");";
        $result=$sqlHandle->command($sqlCommand);            
        return $result;
        }
...
class sqlReturn
    ...
    */
    ...
    public function fetchSelect($format=false)
        {    
        $user_arr = array();      
        $columns=array();
        $printHtml="";
        $printHtml.='<style>'; 
        $printHtml.='.fetchS table,td {align:center;border:1px solid white;border-collapse:collapse;background-color:darkblue;color:white}';
        $printHtml.='.fetchS table    {table-layout: fixed; width: 100%; }';
