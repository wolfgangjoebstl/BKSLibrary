<?
	...
	echo "   RegisterVariable Constants..\n";
	...
	if ($params[0] == 'IPSComponentPlayer_Mediaplayer') 
		{
	   	if (!is_numeric($params[1])) 
			...
			// Create MediaPlayer
			echo "      CreateMediaPlayer ".$mediaPlayerName." in ".$categoryId."\n";
   			$mediaPlayerInstanceId   = CreateMediaPlayer1($mediaPlayerName, $categoryId, 0);
	...
	function CreateMediaPlayer1($Name, $ParentId, $Position=0) {
		$MediaPlayerInstanceId = CreateInstance($Name, $ParentId, "{2999EBBB-5D36-407E-A52B-E9142A45F19C}",$Position);
		echo "WAC_GetDevices ".$MediaPlayerInstanceId." \n";
		$result=IPS_GetConfigurationForm($MediaPlayerInstanceId);
		print_r($result);
		echo "\n\n";
		...
		foreach ($SoundCards as $Idx=>$SoundCard) {
			if ($SoundCard <> "No sound") {
				...
				echo "WAC_SetDeviceID ".$MediaPlayerInstanceId." in ".$Idx."\n";
